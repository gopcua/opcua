...
func (s *Session) handleCreateSessionRequest(cs *services.CreateSessionRequest) {
	...
	case srvStateSessionClosed, srvStateSessionCreated, srvStateSessionActivated:
		...
		s.cfg.signatureToSend = datatypes.NewSignatureDataFrom(cs.ClientCertificate.Get(), cs.ClientCertificate.Get())
	...
	case cliStateCreateSessionSent:
		if cs.ResponseHeader.ServiceResult == 0 {
			...
			*/
			...
			s.cfg.signatureToSend = datatypes.NewSignatureDataFrom(cs.ServerCertificate.Get(), cs.ServerNonce.Get())
...
func (s *Session) CreateSessionRequest() error {
	...
	s.cfg.mySignature = datatypes.NewSignatureDataFrom(s.secChan.cfg.Certificate, nonce)
	return nil
}
...
func validateSignature(received, expected *datatypes.SignatureData) error {
