...
func TestGetEndpointResponse(t *testing.T) {
	...
	if err := srvChan.GetEndpointsResponse(0, datatypes.NewEndpointDescription(
		srvChan.LocalEndpoint(), datatypes.NewApplicationDescription(
			"", "", "", datatypes.AppTypeServer, "", "", []string{""},
		), nil, services.SecModeNone, policyURI, datatypes.NewUserTokenPolicyArray(
			[]*datatypes.UserTokenPolicy{
				datatypes.NewUserTokenPolicy("id", 0, "", "", ""),
			},
		), "", 0,
	)); err != nil {
		t.Fatal(err)
...
func TestFindServerResponse(t *testing.T) {
	...
	if err := srvChan.FindServersResponse(
		0, datatypes.NewApplicationDescription(
			"", "", "", datatypes.AppTypeServer, "", "", []string{""},
		),
	); err != nil {
		t.Fatal(err)
